...
import {
  ...
  Search, Settings, Upload, X, ChevronDown, Clock3, CheckCircle2, AlertCircle,
} from "lucide-react";
import "./App.css";
...
function App() {
  ...
  const scores = data.answers.map(a=>a.score); const avg = average(scores); const med = median(scores);
  const byCategory = categories.map(name => ({ name, value: average(data.answers.filter(a=>a.category===name).map(a=>a.score))! })).filter(x=>x.value);
  const periods = [...new Set(data.answers.map(a=>period(a.date)))].filter(x=>x!=="Undated").map(name => ({ name, value: average(data.answers.filter(a=>period(a.date)===name).map(a=>a.score))! }));
  const filtered = data.verbatims.filter(v => (sentiment === "All" || v.sentiment === sentiment) && (category === "All" || v.category === category) && `${v.content} ${v.question}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <div className="app">
    ...
    <main>
      {page === "dashboard" && <><div className="title"><div><h1>Employee Satisfaction Dashboard</h1><p>Analysis built only from your imported survey responses.</p></div><button className="outline" onClick={()=>setPage("reports")}>Manage imports</button></div>
        {!scores.length ? <Empty title="No imported files yet" detail="Import an Excel, CSV or JSON survey file to populate the dashboard."/> : <>
          <div className="metrics"><Metric label="Overall average" value={`${avg!.toFixed(1)}/4`} sub="Numeric scores use the source 1–4 scale"/><Metric label="Overall median" value={`${med!.toFixed(1)}/4`}/><Metric label="Responses scored" value={String(scores.length)}/><Metric label="Verbatims detected" value={String(data.verbatims.length)}/></div>
          <div className="dashboard-grid"><ChartCard title="Trend in overall ratings" values={periods}/><section className="card categories"><h2>Average by area</h2>{byCategory.map(c=><div className="bar-row" key={c.name}><span>{c.name}</span><div><i style={{width:`${c.value/4*100}%`}}/></div><b>{c.value.toFixed(1)}</b></div>)}</section></div>
          <section className="card"><h2>Average by length of service</h2><div className="seniority">{[...new Set(data.answers.map(a=>a.seniority).filter(Boolean))].map(name=>{const value=average(data.answers.filter(a=>a.seniority===name).map(a=>a.score)); return <div key={name}><i style={{height:`${(value??0)/4*150}px`}}/><span>{name}</span><b>{value?.toFixed(1)}</b></div>})}</div></section>
          <Quotes items={data.verbatims.slice(0,3)} onOpen={setSelected}/></>}
      </>}
      {page === "verbatims" && <><div className="title"><div><h1>Verbatims</h1><p>{data.verbatims.length ? `${data.verbatims.length} real comments detected in imported files` : "No free-text comments have been detected yet."}</p></div></div>
        {!data.verbatims.length ? <Empty title="No verbatims available" detail="Import a survey that includes real free-text comment columns."/> : <><div className="filters"><Search size={20}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search keywords, phrases or topics…"/><Select value={sentiment} onChange={setSentiment} options={["All", "Positive", "Neutral", "Negative"]}/><Select value={category} onChange={setCategory} options={["All", ...categories]}/></div><div className="verbatim-list">{filtered.map(v=><VerbatimCard key={v.id} item={v} onOpen={setSelected}/>)}</div>{!filtered.length && <Empty title="No matching comments" detail="Try clearing some filters."/>}</>}
      </>}
